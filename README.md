...
- [@ff-labs/fff-node](https://www.npmjs.com/package/@ff-labs/fff-node) by Dmitry Kovalenko, with override direction informed by [@ff-labs/pi-fff](https://www.npmjs.com/package/@ff-labs/pi-fff) — bundled canonical `find`/`grep` retrieval and workspace `@` suggestions. Pho Code owns the hardened adapter and index location; it does not load the Pi TUI extension.
