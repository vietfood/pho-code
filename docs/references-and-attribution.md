...
| `packages/ui/src/transcript.tsx`, `tool-row.tsx`, `thinking-block.tsx`, `tool-presentation.ts`, `work-entry-icon.tsx` | `refs/t3code/apps/web/src/components/chat/MessagesTimeline.tsx` (`PlainWorkEntryRow`, thinking tone, WorkingTimelineRow) | `6bc6cb6be4ebecd5bd7a3b7b88f07cb61815ded5` | Work-entry chrome: icon wrappers, owner-facing titles without a status word, muted previews, check/x status, indented expand body, thinking rows, working dots; omitted LegendList, diffs, agent spawn CTA, tooltips. Pho/Lucide/CodeX/Meteocons icon packs and expanded tool panels are harness-owned polish on top of that chrome. | MIT; recorded here | chat Electron spec + UI unit tests |
...
| `packages/ui/src/provider-icon.tsx`, `backend-icon.tsx`, `lib/lobe-brand-icons.ts`, `lib/lobe-color-icons.ts` | [`@lobehub/icons-static-svg`](https://www.npmjs.com/package/@lobehub/icons-static-svg) `1.94.0` ([lobehub/lobe-icons](https://github.com/lobehub/lobe-icons)) | 1.94.0 / 2026-08-27 | Runtime SVG assets. Settings Appearance Brand marks: Mono paints `currentColor` via CSS mask; Color uses Lobe `-color.svg` / `-brand-color.svg` as `<img>` on a light plate with a hairline so lime/white logos stay visible on cream. Brands without a color file stay on the mono mask. Pi aliases such as `openai-codex` → Codex and `cloudflare-workers-ai` → Workers AI. Hyphenated regional/plan ids fall back to a prefix. OpenRouter catalog ids always keep the OpenRouter mark. Not `@lobehub/icons` (antd / `@lobehub/ui` peers). | MIT (LobeHub); recorded here | UI unit provider-icon / backend-icon / conversation tests |
| `packages/ui/src/lib/work-entry-codex.tsx` | [`@codexteam/icons`](https://github.com/codex-team/icons) `0.3.3` | 0.3.3 / 2026-08-27 | Settings Appearance pack `codex-team` (label CodeX) for tool/thinking work-entry glyphs. Stroke SVGs use `currentColor`. GitHub rows stay on the Lobe GitHub mark. Id avoids colliding with the Codex backend. | MIT (CodeX); recorded here | UI unit work-entry-icon test |
| `packages/ui/src/lib/work-entry-meteocons.tsx` | [`@meteocons/svg-static`](https://github.com/basmilius/meteocons) `0.1.0` | 0.1.0 / 2026-08-27 | Settings Appearance pack `meteocons`. Uses the colorful **fill** set as `<img>` (not monochrome masks). Artwork is optically scaled inside the same 14px Lucide slot (`overflow: hidden`) so padding in the 128 viewBox does not overflow the row. Thought is starry-night; run is lightning-bolt. GitHub rows stay on the Lobe GitHub mark. | MIT (Bas Milius); recorded here | UI unit work-entry-icon test |
| `packages/ui/src/skill-source-icon.tsx` Codex/Claude/Cursor marks | Lobe Icons via `ProviderIcon` | 1.94.0 / 2026-08-27 | Codex and Claude skill sources use the Codex/Claude marks; Cursor reuses the Cursor mark; Pho Code/Pi stay monograms | MIT (LobeHub); recorded here | UI unit skills-settings test |
| `packages/ui/src/provider-icon.tsx` (retired) | [Simple Icons](https://simpleicons.org/) brand path data (`deepseek`, `anthropic`, `openai`, `googlegemini`, `cursor`, …) via jsDelivr `simple-icons` package | retrieved 2026-08-13; `cursor` path reused 2026-08-14; retired 2026-08-27 | Was inlined monochrome `currentColor` SVG paths keyed by Pi provider ids with a letter fallback; replaced by `@lobehub/icons-static-svg` | CC0 1.0; recorded here | UI unit conversation / provider-icon test |
| `packages/ui/src/skill-source-icon.tsx` Cursor mark (retired) | [Simple Icons](https://simpleicons.org/) `cursor` path via `ProviderIcon` | retrieved 2026-08-14; retired 2026-08-27 | Cursor/Codex/Claude skill marks now come from Lobe Icons | CC0 1.0; recorded here | UI unit skills-settings test |
| `packages/ui/src/assets/openai-codex-light.png`, `openai-codex-dark.png` (removed 2026-08-27) | Owner-supplied OpenAI Codex product marks (black cloud for light UI, white cloud for dark UI) | 2026-08-13; removed 2026-08-27 | Were shown for Pi provider id `openai-codex` only; replaced by the Lobe Codex SVG | OpenAI trademark; personal identification use, not redistributed as a standalone asset | UI unit provider-icon test |
...
| Composer highlight ring + `/` skill picker in `packages/ui/src/composer.tsx`, `composer-picker-menu.tsx` | [Beautiful UI](https://www.beautifului.dev/) PromptBar.tsx (`#prompt-bar`); [globals.css](https://github.com/slev12397/beautiful-ui/blob/main/app/globals.css) `pop-in` | retrieved 2026-08-13 / 2026-08-22 | Shell ring for `@` mention and `/` skill insert (max-thinking ring removed); gliding highlight and pop-in `@` / `/` menus with a type-to-search footer; omitted dictation, `glimm` rainbow sweep, autoplay, and fake source/command catalogs | MIT (Shane Levine); recorded here | UI unit composer-highlight + composer-tokens + composer-picker-menu tests |
...
| `packages/runtime` local retrieval (`local-retrieval.ts`, `retrieval-feature.ts`) | [`@ff-labs/fff-node`](https://www.npmjs.com/package/@ff-labs/fff-node) `0.10.1`; override direction informed by [`@ff-labs/pi-fff`](https://www.npmjs.com/package/@ff-labs/pi-fff) | 0.10.1 / 2026-08-28 | Application-owned `FileFinder` adapter replaces canonical Pi `find`/`grep` and supplies composer `@` suggestions from the same index. The implementation keeps Pi's input names but owns its constraint validation, ignored-path fallback, formatting, and bounds; `pi-fff` code is not copied or loaded. Indexes and frecency DBs live under app data `retrieval/`, not Pi's default `~/.pi/agent/fff/`. | MIT (Dmitry Kovalenko); notices via `fff-node` | runtime local-retrieval, feature registration, constraint, ignored-path, desktop, and packaged tests |
