...
| `packages/ui/src/loading-dots.tsx`, `session-leading-mark.tsx` | [Beautiful UI](https://www.beautifului.dev/) Loading State Dots variant (`#loading-state`) | retrieved 2026-08-14 | Compact three-dot pulse in the session row while a run is live; omitted pixel-grid, Drive, Orbit, shimmer label, and elapsed timer | MIT (Shane Levine); recorded here | UI unit session-leading-mark tests |
