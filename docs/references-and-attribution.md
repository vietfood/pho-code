...
| `packages/ui/src/change-review-sheet.tsx`, `right-sidebar.tsx`, `lib/change-review-diff.ts`, `.change-review-*` in `theme.css` | `refs/t3code/apps/web/src/components/files/FilePreviewPanel.tsx`, `DiffPanelShell.tsx`, `diffs/StyledDiffCodeView.tsx`, `chat/DiffStatLabel.tsx`, `chat/PanelLayoutControls.tsx`, `RightPanelTabs.tsx`, `index.css` `--code-background` / hunk separators | `6bc6cb6be4ebecd5bd7a3b7b88f07cb61815ded5` | Unified-diff card with file verb, +/- stats, line gutter, unmodified-line pills; persistent right sidebar as a collapsed overlay pill (PanelRight, FileDiff, Context prompt) that expands to a resizable panel; omitted Pierre Diffs, comments, before/agent/current tabs, explorer, syntax-colored diff tokens, and extra rail surfaces (terminal, files, browser) | MIT; recorded here | UI unit change-review-sheet + right-sidebar tests |
