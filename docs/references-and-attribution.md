...
| `packages/ui/src/shiki-code.tsx`, `shiki-highlight.ts` | `refs/t3code/apps/web/src/components/ChatMarkdown.tsx` Shiki path + `lib/syntaxHighlighting.ts` | `6bc6cb6be4ebecd5bd7a3b7b88f07cb61815ded5` | Settled-only highlight with Map cache and language fallback; omitted Suspense/`use()`, Pierre Diffs highlighter; copy lives in shared codeblock chrome | MIT; recorded here | UI unit theme helper + markdown tests |
...
| `packages/ui/src/provider-icon.tsx` | [Simple Icons](https://simpleicons.org/) brand path data (`deepseek`, `anthropic`, `openai`, `googlegemini`, …) via jsDelivr `simple-icons` package | retrieved 2026-08-13 | Inlined monochrome `currentColor` SVG paths keyed by Pi provider ids; letter fallback for unknown providers; no runtime dependency on `simple-icons` | CC0 1.0; recorded here | UI unit conversation test |
...
| `packages/ui/src/loading-state.tsx`, live waiting chrome in `transcript.tsx` / `work-log-toggle.tsx` | [Beautiful UI](https://www.beautifului.dev/) LoadingState.tsx (`#loading-state`) | retrieved 2026-08-13 | Pixel-grid Drive/Dots/Orbit loader, shimmer label, tenths elapsed; parent owns the clock; tokens mapped to Pho CSS; demo autoplay omitted | MIT (Shane Levine); recorded here | UI unit loading-state + conversation tests |
...
| Composer highlight ring + `/` stub in `packages/ui/src/composer.tsx` | [Beautiful UI](https://www.beautifului.dev/) PromptBar.tsx (`#prompt-bar`) | retrieved 2026-08-13 | Shell ring for max thinking, `@` mention, and `/` skill stub; omitted dictation, `glimm` rainbow sweep, autoplay, and fake source/command catalogs | MIT (Shane Levine); recorded here | UI unit composer-highlight + conversation tests |
| `packages/ui/src/streaming-output.tsx`, `lib/smooth-stream.ts`, `lib/rehype-stream-tail.ts` | [Beautiful UI](https://www.beautifului.dev/) StreamingText.tsx (`#streaming-text`) | retrieved 2026-08-13 | Word-paced catch-up (55ms), last-word `stream-in` blur, thin caret; omitted citations, follow-ups, source chips, and demo action row; live markdown and reduced-motion snap retained | MIT (Shane Levine); recorded here | UI unit smooth-stream + markdown tests |
...
| `packages/runtime` pho-web (`web-url.ts`, `web-client.ts`, `web-feature.ts`) | Policy and DuckDuckGo HTML parse informed by [`pi-web-access`](https://www.npmjs.com/package/pi-web-access) `0.22.0` (`ssrf-protection.ts`, `duckduckgo.ts`); extraction via `@mozilla/readability` `0.6.0`, `linkedom` `0.16.11`, `turndown` `7.2.1` | 0.22.0 / 2026-08-13 | Application-owned `pho-web` adapter exposing only `web_search` (DuckDuckGo) and `fetch_content` (public HTTP GET). `pi-web-access` is not loaded (TUI, Exa/Codex fallback, GitHub clone, PDF/video, cookies). SSRF, redirect, timeout, and size limits are harness-owned. | MIT (Nico Bailon / Mozilla / linkedom / turndown); notices via pinned extract packages | runtime web-url tests |
