...
| Live stream-tail + caret in `stream-text.tsx`, `lib/stream-text.ts`, `transcript.tsx`, `thinking-block.tsx`, `.stream-tail` / `.stream-caret` in `theme.css` | [Beautiful UI](https://www.beautifului.dev/) [globals.css](https://github.com/slev12397/beautiful-ui/blob/main/app/globals.css) StreamText `.stream-tail` / `.stream-caret`; harness `StreamLine` caret | retrieved 2026-08-22 | Newest live characters get the masked blur; caret is solid while tokens arrive. Assistant GFM keeps markdown markers in ConservativeMarkdown. Tokens still come from Pi; omitted the demo word-by-word timer, citations, and follow-ups | MIT (Shane Levine); recorded here | UI unit stream-text + thinking-block + conversation tests |
