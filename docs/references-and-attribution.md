...
| `packages/runtime/src/ask-user-question.ts`, `plan-agent-feature.ts` prompt guidelines | juicesharp `@juicesharp/rpiv-ask-user-question` `2.6.0` `tool/validate-questionnaire.ts`, `tool/response-envelope.ts`, `tool/format-answer.ts` | 2.6.0 / 2026-08-16 | Pho-owned validation, reserved labels, decline vs host-failure envelope, and tool guidelines. 8 KiB fields reject. Package and `pi-tui` not baked. | MIT (juicesharp); recorded here | protocol + runtime ask-user unit tests |
| `packages/runtime/src/ask-user-rpc-fallback.ts` | juicesharp `@juicesharp/rpiv-ask-user-question` `2.6.0` `rpc-fallback.ts` | 2.6.0 / 2026-08-16 | Sequential `select`/`input` walker used only when the questionnaire card cannot render | MIT (juicesharp); recorded here | runtime ask-user unit tests |
...
| `packages/ui/src/change-review-sheet.tsx`, `right-sidebar.tsx`, `lib/change-review-diff.ts`, `.change-review-*` in `theme.css` | `refs/t3code/apps/web/src/components/files/FilePreviewPanel.tsx`, `DiffPanelShell.tsx`, `diffs/StyledDiffCodeView.tsx`, `chat/DiffStatLabel.tsx`, `chat/PanelLayoutControls.tsx`, `RightPanelTabs.tsx`, `index.css` `--code-background` / hunk separators | `6bc6cb6be4ebecd5bd7a3b7b88f07cb61815ded5` | Unified-diff card with file verb, +/- stats, line gutter, unmodified-line pills, in-sheet search, whitespace glyphs, bounded context control, and token-colored lines as React text; persistent right sidebar as a collapsed overlay pill (FileDiff, Context prompt) that expands to a resizable panel; re-clicking the active surface collapses it (no PanelRight control); omitted Pierre Diffs, comments, before/agent/current tabs, explorer, and extra rail surfaces (terminal, files, browser) | MIT; recorded here | UI unit change-review-sheet + right-sidebar tests |
