...
- [Canonical FFF retrieval](./2026-08-28-defect-canonical-fff-retrieval.md) — accepted 2026-08-28. The bundled workspace index now owns the single canonical `find` and `grep` registrations; parallel legacy ids and multi-pattern code are gone, scoped results are post-filtered, explicitly requested ignored paths use a contained temporary index, and result/context/time/byte bounds are enforced.
