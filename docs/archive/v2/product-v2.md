...
- Web access uses a source-controlled `pho-web` adapter informed by reviewed `pi-web-access` `0.22.0`. Search fans out in parallel across keyless DuckDuckGo HTML/Lite, Bing, Brave, Mojeek, and Jina, then merges unique URLs. Fetch is application-owned bounded HTTP(S) extraction, with YouTube watch/shorts URLs returning public captions and metadata when available, and Jina Reader for thin JS pages. Jina discloses the query or URL to jina.ai. `pi-web-access` and `supi-web` are not loaded. Gemini cookie/API video understanding, Exa MCP, Codex search, authenticated browser automation, GitHub cloning, PDF conversion, form submission, file upload, and arbitrary browsing remain outside the milestone.
