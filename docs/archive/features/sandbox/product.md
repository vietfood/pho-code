...
- Process separation (Phase F) is not this feature. Accepted window-first app launch (create the Electron window before Pi `ModelRuntime.create`) is also not this feature; its evidence lives under archived [`window-first-pi-core`](../../urgent/window-first-pi-core/README.md).
