...
Status: **Pending**. V4 is not accepted and Pho Code is not a public-beta release. The owner cannot enroll in the Apple Developer Program, so Developer ID signing, notarization, Gatekeeper-clean DMG, and Homebrew cask distribution cannot be verified. Remaining milestones are held. Milestone 0's source freeze and fail-closed proof packager remain in the repository. Do not resume V4, and do not emit an unsigned public artifact, until Developer ID authority exists. Evidence: [`logs/2026-08-20-hold-pending-apple-developer.md`](./logs/2026-08-20-hold-pending-apple-developer.md).

Independent add-ons, conversation UI, and a later numbered version (V5) may proceed. They must not absorb this contract. Current accepted behavior remains in [`../../current-state.md`](../../current-state.md) and [`../../architecture/`](../../architecture/README.md).
