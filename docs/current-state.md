...
- an Agent-chat `ask_user_question` questionnaire card and per-chat Plan/Agent toggle with a Plan sidebar document plus session `todo` list (Plan/Agent Milestones 0–2; in source, not accepted); `ask_user_question`, `update_plan_document`, `todo`, and `execute_plan` are permission-allow-listed so they do not open a permission dock; Execute is the Plan footer button **or** `execute_plan` after the owner asks in chat; the Execute kickoff is hidden from the transcript; managed profiles allow `web_search` and ask for `fetch_content`;
...
- **Plan / Agent and ask-user add-on:** in implementation; owner-approved 2026-08-16. Milestone 0 ask-back, Milestone 1 Plan/Agent (composer toggle, write tools off in Plan, Plan sidebar document, Execute), and Milestone 2 session todos (`todo` tool, composer `n/m` chip, Plan checklist) are in source and not accepted. The Plan rail renders settled chat markdown (KaTeX/mermaid/Shiki), todos under the document, a pen-icon source editor, and one comment box (no Stay/Refine buttons). Product and plan live under [`features/plan-agent`](./features/plan-agent/README.md). Evidence: [`features/plan-agent/logs/2026-08-16-m0-ask-back.md`](./features/plan-agent/logs/2026-08-16-m0-ask-back.md), [`features/plan-agent/logs/2026-08-16-m1-plan-agent.md`](./features/plan-agent/logs/2026-08-16-m1-plan-agent.md), [`features/plan-agent/logs/2026-08-16-m2-todos.md`](./features/plan-agent/logs/2026-08-16-m2-todos.md), [`features/plan-agent/logs/2026-08-16-feedback-ask-user-allow.md`](./features/plan-agent/logs/2026-08-16-feedback-ask-user-allow.md), [`ui/logs/2026-08-16-feedback-plan-rendered-markdown.md`](./ui/logs/2026-08-16-feedback-plan-rendered-markdown.md), [`ui/logs/2026-08-16-feedback-plan-comment-icons.md`](./ui/logs/2026-08-16-feedback-plan-comment-icons.md).
- **Agent-tool sandbox add-on:** accepted 2026-08-17. Default on; fail-closed Seatbelt wrap for agent bash; healthy sandbox skips bash and in-policy `read`/`write`/`edit` asks; extra-workspace writes may still ask on `external_directory`; packaged macOS stages the pinned engine and bundled `rg`. Product and plan live under [`features/sandbox`](./features/sandbox/README.md). Review: [`features/sandbox/logs/2026-08-17-acceptance-review.md`](./features/sandbox/logs/2026-08-17-acceptance-review.md). Distinct from Phase F runtime extraction and from the urgent window-first track.
