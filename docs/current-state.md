...
- **Milestone 3 — session continuity and lifecycle:** implementation complete pending verification and owner real-provider acceptance. Independently owned session controllers, keyed catalog/cache, sidebar activity, archive/restore, recoverable OS-Trash chat removal, and per-workspace FFF indexes are in source. Desktop and packaged journeys for background-run switching, archive persist, busy-state Trash refusal, and settled Trash are added. Owner-verified live-provider background switching remains the acceptance gate.
