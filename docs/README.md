...
Pho Code V1 `0.1.0` is an independently owned Rust harness with one DeepSeek API backend, a stable `pho` command surface, and a native four-pane GPUI workbench over the same runtime. It owns API-key custody, direct streamed transport, context and tool continuation, first-party tools, approvals, sessions, recovery, and presentation projection. [ADR 0003](decisions/0003-deepseek-api-first-backend.md) records the backend/runtime boundary, [ADR 0004](decisions/0004-native-workbench-phase-6.md) records the expanded native surface, and [ADR 0005](decisions/0005-release-v1-and-defer-phase-6b.md) records the first-release and deferred Phase 6B boundary.
...
| Native four-pane workbench scope, retained V1 execution limits, read-only viewer, PTY and rendering boundary | [ADR 0004](decisions/0004-native-workbench-phase-6.md) | Accepted decision snapshot |
...
| Four-pane shell, native presentation projections, Markdown/LaTeX, activity, composer, and workbench-service boundaries | [GPUI workbench](architecture/gpui-workbench.md) | Normative V1 architecture; released in 0.1.0 |
| Native entry/startup/shutdown, preferences, layout, focus, accessibility, theme/assets | [Native workbench lifecycle](architecture/native-workbench-lifecycle.md) | Normative V1 architecture; released in 0.1.0 with Phase 6B qualification |
...
| User PTY/emulator actor, process/input/output/resize/privacy/shutdown | [User terminal](architecture/user-terminal.md) | Normative V1 architecture; single-surface V1 with multi-tab Phase 6B work |
...
| Phase order, status, and completion rules | [Implementation roadmap](implementation/README.md) | V1 `0.1.0` released; V2 Phase 6B ready |
...
| Native completion, compaction, subagents, second backend/portability | [V2 roadmap](implementation/v2/README.md) | Phase 6B ready; later expansion reserved |
