...
Startup ordering and crash isolation of in-process Pi are **not** shell changes. Archived [`window-first-pi-core`](../archive/urgent/window-first-pi-core/README.md) owns the accepted same-process startup order: load metadata/IPC/window before dynamically importing and constructing Pi. Its desktop behavior is verified; packaged behavior and elapsed-time measurements were explicitly not verified at closure. Extracting the complete `HarnessRuntime` into another process is promoted but unimplemented under pending [`V4 — Public Beta Foundation`](../version/v4/README.md). Bounded Stop, Stop-all, and bounded teardown are already accepted under archived [`agent-stop`](../archive/urgent/agent-stop/README.md); neither they nor window-first ordering can survive a frozen main-process Pi.
