...
- `skill-invoke` registers `read_skill` over `SkillSourceRegistry`. The tool description lists invocable skills (name + description, no paths). The agent may load one when the task matches; `/` remains an owner shortcut that expands the selected text skill on send. Skills with `disable-model-invocation: true` stay `/`-only;
...
- `approval-modes` intercepts every Pi tool call through the shared Pho Agent broker. Pho Code injects product invariants, contained policy, permission ask evidence, reviewer settlement, and the exact owning dispatch adapter.
...
`pi-cursor-sdk` `0.2.0` is baked the same way: pin the exact package, stage it under app-owned `Resources/features/pi-cursor-sdk` with nested `@cursor/sdk` (and the current-platform `@cursor/sdk-*` binary package), and load only through the manifest. Harness policy forces `PI_CURSOR_RUNTIME=local` and `PI_CURSOR_SETTING_SOURCES=none` so Cursor Cloud and ambient `~/.cursor` MCP/plugins/rules stay out of product scope. The extension still registers on every Pi session; Cursor SDK tools (`cursor_ask_question`, `cursor_activate_skill`, `cursor`) are active, listed in Context prompt, and compiled into prompt A only while the live session model is Cursor (`provider === "cursor"`). Switching the session model re-applies that tool policy. Cursor host tools are not gated by the permission-system feature; bridged Pho Code tools still are. Settings → Accounts lists Cursor at harness boot (API-key import only) so a key can be added before any chat. `pi-cursor-sdk` still registers a placeholder so Pi will load fallback `cursor/*` models; Pho Code hides those models, refuses selecting/sending on them, and will not project Cursor as the session model until a stored Cursor credential or a real `CURSOR_API_KEY` exists. Selecting a `cursor/*` model then shows a warning dialog before the session model binding changes. Cursor Desktop/CLI login is not supported by `@cursor/sdk`—only a Dashboard or service-account API key.
...
The application-owned local-retrieval feature `2.0.0` wraps pinned `@ff-labs/fff-node` `0.10.1`. One `FileFinder` per active workspace replaces Pi's built-in `find` and `grep` registrations and serves the typed `searchWorkspaceReferences` command. The replacement keeps Pi's canonical tool ids and input names, so a session exposes one file finder and one content search rather than parallel built-in and FFF variants. There is no multi-pattern retrieval tool; regex alternation through `grep` covers that use without another schema or cursor store.

The adapter treats a supplied `path` as a workspace boundary and post-filters every native result instead of trusting FFF's inline constraint parser. A concrete in-workspace path absent from the main git-aware index gets a temporary no-watch finder rooted at that file's parent or directory; this makes an explicitly requested ignored path searchable without broadening to adjacent files. Symlink-resolved paths must remain inside the workspace. `find` returns at most 100 paths; `grep` returns at most 100 matches, clamps context to five lines on either side, truncates individual lines, stops native search after five seconds, and caps final UTF-8 output at 200 KiB. Abort is checked before and after native work; the synchronous native call itself cannot be interrupted mid-call.

Pho Code does not load `@ff-labs/pi-fff` because that extension owns a second index and TUI `@` autocomplete this host no-ops. It also does not use Pi's on-demand `rg`/`fd` acquisition path for canonical retrieval. Index and frecency state live under application data `retrieval/<workspace-hash>/`, never in packaged resources or another Pi installation's default FFF database. Native lookup failure degrades the feature with a named diagnostic; ordinary chat continues.
...
The normal product has no generic feature Reload action because the executable manifest is immutable for the running build. Milestone 4's explicit **Refresh skills** action rereads only enabled fixed text-skill roots and recomputes provenance/compatibility for the `/` catalog, the advertised `read_skill` list, and named load. It does not rebind session controllers or change a running prompt. Applying another supported feature setting may internally reload/rebind idle sessions; that lifecycle action is not exposed as generic composition control. Development may restart Electron to pick up built-in feature source changes. A future signed app update may replace the whole baked feature bundle; it must not mutate feature code inside a running session.
...
The permission engine is rule-based rather than scalar. Its `guarded`, `balanced`, `developer`, Custom, and YOLO states remain compatibility and migration inputs, but the ordinary owner surface now presents Ask for approval, Approve for me, Full access, and the active boundary. Unmatched policies and a shared agent root remain Ask-only until explicit migration. Migration preserves fields outside the typed adapter, refuses invalid/unrecognized config, and is atomic; it never infers Auto or Full. Do not show `doublePressToConfirm`; the pinned package documents it as TUI-only and it does not affect this RPC/frontend host.

The managed policies remain the compatibility templates. Runtime start writes the harness allow list and managed web pair, then registers the named sandbox and approval authorizers only when their owning runtime is active. Permission asks are bounded evidence for product policy; a package allow can never replace whole-action interception, and an approval cannot override a package deny. Healthy Seatbelt wrapping skips duplicate bash/in-policy file asks without converting denies. Managed profiles explicitly allow `/dev/null` as a safe sink so sandboxed network-denial commands do not create a legacy path prompt. See [`../archive/features/sandbox`](../archive/features/sandbox/README.md) and the active [`../features/approval-modes`](../features/approval-modes/README.md).
...
The Plan/Agent factory implements an extra structured `questionnaire` request plus Plan/Agent tool policy, a session `todo` tool, and Plan-only `execute_plan`: JSON-safe questions/answers on the same one-dialog lifecycle, distinct chrome from permission copy; Plan drops `write` / `edit` / `move_to_trash` / Cursor SDK tools. `custom` and `editor` still throw. See [`../archive/features/plan-agent`](../archive/features/plan-agent/README.md).
