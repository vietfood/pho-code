...
The internal `developer` policy remains the v2 daily-driver template: ordinary workspace reads/edits, reviewed inspection and validation command families, and the application-owned `move_to_trash` tool are allowed inside the selected workspace. Public `web_search` is allowed; `fetch_content` asks because it performs an HTTP GET. `ask_user_question`, `update_plan_document`, `todo`, and `execute_plan` are always allowed — they are the owner prompt, not a grant. Runtime start writes those harness allows (and the managed web pair) onto an existing managed config so `"*": "ask"` cannot catch a tool that Settings already treats as part of the preset. Every managed template explicitly denies permanent-removal commands with the reason “Permanent removal is unavailable. Use the move_to_trash tool.” Sensitive paths, external directories, privilege escalation, and publication remain asked or denied.
